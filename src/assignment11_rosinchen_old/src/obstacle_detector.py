...
class ObstacleDetector:
	...
	def process_obstacles(self, obstacles, model, position):
		...
		distanceToTrack = np.array([np.linalg.norm(nearestPoints_obs[i]-obstacles[i]) for i in range(len(nearestPoints_obs))])
		...
		if self.logging:
			...
			print("distanceToCar[nearestObstacle] < self.distanceToObstacleTreshold: ", distanceToCar[nearestObstacle] < self.distanceToObstacleTreshold)
